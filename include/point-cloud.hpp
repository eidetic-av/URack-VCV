...
struct PointCloudPort : app::SvgPort {
  ...
  PointCloudPort() {
    setSvg(
        APP->window->loadSvg(asset::system("res/ComponentLibrary/PJ301M.svg")));
  }
  ...
  void onDragStart(const event::DragStart &e) override {
    ...
    if (cw && (APP->window->getMods() & RACK_MOD_MASK) == GLFW_MOD_ALT) {
      if (!settings::cableColors.empty()) {
        // NVGcolor doesn't implement an equality operator...
        // So we need to check each union value
        auto colorValue = cw->color.rgba;
        int colorId = -1;
        for (uint id = 0; id < settings::cableColors.size(); id++) {
          auto checkValue = settings::cableColors[id].rgba;
          if (colorValue[0] != checkValue[0])
            continue;
          if (colorValue[1] != checkValue[1])
            continue;
          if (colorValue[2] != checkValue[2])
            continue;
          if (colorValue[3] != checkValue[3])
            continue;
          colorId = id;
          break;
        }
