...
struct UModule : Module {
        ...
        if (!URack::Settings::initialised) {
            ...
            for (auto plugin : rack::plugin::plugins)
                if (plugin->slug == "URack-Collection") {
                    ...
                    auto managerPtr = reinterpret_cast<URack::NetworkManager **>(managerSym);
                    URack::networkManager = *managerPtr;

                    void *instanceListSym = dlsym(handle, "moduleInstanceList");
                    auto instanceListPtr = reinterpret_cast<std::map<std::string, URack::UModule *> **>(instanceListSym);
                    UModule::moduleInstances = *instanceListPtr;

                    void *defaultHostsSym = dlsym(handle, "defaultHostSelections");
                    auto defaultHostsPtr = reinterpret_cast<std::vector<int>**>(defaultHostsSym);
    ...
    void configUpdate(std::string oscAddress, int param, int input = -1,
        ...
        if (param > -1) configParam(param, minValue, maxValue, defaultValue, oscAddress);
    ...
    void activateHost(std::string ip, int port = SENDPORT) {
        ...
        if (!existingHost) {
            activeHosts.push_back(URack::networkManager->dispatcher->connect_host(ip, port));
            URack::settings->save();
        }
    ...
    void deactivateHost(std::string ip, int port = SENDPORT) {
        for (unsigned int i = 0; i < URack::networkManager->dispatcher->sockets.size(); i++) {
            auto socket = URack::networkManager->dispatcher->sockets[i];
            if (socket->ip == ip && socket->port == port) {
                activeHosts.erase(std::remove(activeHosts.begin(), activeHosts.end(),
                                              socket->hostNum), activeHosts.end());
                break;
            }
        }
    }
    ...
    void process(const ProcessArgs &args) override {
        if (!initialised) {
            ...
            URack::networkManager->dispatcher->send(activeHosts, instanceAddress + "/Active", active);
        ...
        if (activateParam != -1) {
            ...
            if (activateInput == -1 || !inputs[activateInput].isConnected()) {
                ...
                if (activateTrigger.process(activeButton)) active = active > 0 ? 0 : 1;
            ...
            if (oldActive != active) URack::networkManager->dispatcher->send(
                    activeHosts, instanceAddress + "/Active", active);
        }
        if (activateLight != -1) lights[activateLight].setBrightness(active * 10.f);
        ...
        if (updateTimer >= OSC_UPDATE_PERIOD) {
            ...
            for (unsigned int i = 0; i < updateParams.size(); i++) {
                ...
                if (sendUpdate) {
                    ...
                    URack::networkManager->dispatcher->send(activeHosts,
                                                            address, update);
                ...
                if (cables.size() > output.connections.size()) {
                    ...
                    for (auto cableWidget : cables) {
                        ...
                        if (inputPort) {
                            ...
                            std::vector<OscArg> update = {output.port->oscAddress.c_str(), module->id,
                    ...
                    for (auto target : output.connections) {
                        ...
                        for (auto cableWidget : cables) {
                            auto inputPort = dynamic_cast<PointCloudPort *>(
                                cableWidget->inputPort);
                            if (inputPort == target) {
                                stillConnected = true;
                                break;
                            }
                    ...
                    for (auto target : disconnectList) {
                        ...
                        output.connections.erase(
                            std::remove(output.connections.begin(),
                                        output.connections.end(), target),
                            output.connections.end());
                        // and dispatch a disconnect message
                        auto targetModule =
                            dynamic_cast<UModule *>(target->module);
                        std::vector<OscArg> update = {
                            output.port->oscAddress.c_str(), targetModule->id,
                            target->oscAddress.c_str()};
                        std::string address = instanceAddress + "/Disconnect";
                        URack::networkManager->dispatcher->send(
                            activeHosts, address, update);
                    }
                }
...
struct UModuleWidget : ModuleWidget {
    ...
    void addPointCloudOutput(math::Vec pos, UModule *module, int outputId,
        ...
        port->oscAddress = oscAddress;
        if (module) module->pointCloudOutputs.push_back({outputId, false, port,
                                                 std::vector<PointCloudPort *>{}});
    }
    ...
    struct HostMenuItem : MenuItem {
        ...
        void onAction(const event::Action &e) override {
            if (!active) module->activateHost(socketInfo->ip);
            else module->deactivateHost(socketInfo->ip);
        }
    };
